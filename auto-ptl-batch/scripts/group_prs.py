...
def get_pr_detail(pr_num):
    return github_api.gh_get(f'https://api.github.com/repos/ceph/ceph/pulls/{pr_num}')


def get_pr_files(pr_num):
    """Return frozenset of changed file paths for a PR."""
    files = github_api.gh_paginate(
...
def path_at_depth(filepath, depth):
    ...
    e.g. 'src/rgw/multisite/sync.cc' at depth=3 -> 'src/rgw/multisite'
...
def main():
    ...
    for (component, branch), pr_nums in sorted(groups.items()):
        ...
        # Exclude documentation-only PRs: they need no teuth suite
