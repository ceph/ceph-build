...
if [ "$OS_PKG_TYPE" = "rpm" ]; then
  ...
  find release/${VERSION}/rpm/*/SRPMS | grep rpm | $VENV/bin/chacractl binary ${chacra_flags} create ${chacra_endpoint}/source/flavors/${FLAVOR}
  find release/${VERSION}/rpm/*/RPMS/* | grep rpm | $VENV/bin/chacractl binary ${chacra_flags} create ${chacra_endpoint}/${ARCH}/flavors/${FLAVOR}
  ...
  PACKAGE_MANAGER_VERSION="${VERSION}-1${OS_VERSION}"
  find ${WORKSPACE}/dist/ceph/ | \
    egrep "*(\.changes|\.deb|\.ddeb|\.dsc|ceph[^/]*\.gz)$" | \
    egrep -v "(Packages|Sources|Contents)" | \
    $VENV/bin/chacractl binary ${chacra_flags} create ${chacra_endpoint}
  if [ -f release/${VERSION}/cephadm_${VERSION}*.deb ] ; then
    ...
    echo cephadm | $VENV/bin/chacractl binary ${chacra_flags} create ${chacra_endpoint}
  fi
fi
