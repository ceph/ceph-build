...
#
# --no-reimage is what makes --lock usable here: by default it reimages
# bare-metal nodes whose machine_type is a reimage type, which is the opposite
# of what phase_deploy wants (teuthology PR for the flag: branch
# teuthology-lock-no-reimage-11574f).  Until that lands in teuthology's main,
# run this job with TEUTHOLOGYBRANCH=teuthology-lock-no-reimage-11574f.
...
# drop the lock.  There is no CLI equivalent: teuthology-lock --unlock routes
# every bare-metal node through unlock_one, which calls stop_node and powers
# it off.  These nodes were just imaged and verified, so hand them back
# running, the way this phase always did.
funRelease () {
  if [ $# -eq 0 ]; then
    return 0
  fi
...
sys.exit(0 if ops.unlock_many(sys.argv[1:], get_user()) else 1)
' "$@"
}
